  ...
  try {
    ...
    try {
      const userResponse = await fetch(
        `${API_BASE_URLS.user}/api/users/register`,
        {
          method: "GET",
    ...
    try {
      const catalogResponse = await fetch(
        `${API_BASE_URLS.catalog}/api/catalog/yachts`,
        {
          signal: AbortSignal.timeout(3000),
        }
      );
...
export const authAPI = {
  ...
  async login(credentials: LoginRequest): Promise<LoginResponse> {
    try {
      console.log(
        "Intentando hacer login en:",
        `${API_BASE_URLS.user}/api/users/login`
      );
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          throw new Error(
            `Error en el login: ${response.status} ${response.statusText}`
          );
        }
      }
      ...
      console.log(
        ...
        loginData.token ? `${loginData.token.substring(0, 10)}...` : "No token"
      );
      ...
      if (
        ...
        throw new Error(
          `No se pudo conectar al servidor de autenticación. Verifica que el backend esté ejecutándose en ${API_BASE_URLS.user}`
        );
      }

      if (error instanceof Error && error.name === "AbortError") {
        throw new Error(
          "La conexión tardó demasiado. Verifica tu conexión a internet e intenta nuevamente."
        );
      }
  ...
  async getCurrentUser(token: string): Promise<User> {
    try {
      console.log(
        "Intentando obtener usuario actual desde:",
        `${API_BASE_URLS.user}/api/users/me`
      );
      console.log(
        "Token:",
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          throw new Error(
            `Error al obtener usuario: ${response.status} ${response.statusText}`
          );
        }
      }
      ...
      if (
        ...
        throw new Error(
          `No se pudo conectar al servidor. Verifica que el backend esté ejecutándose en ${API_BASE_URLS.user}`
        );
      }
      ...
      if (error instanceof Error && error.message.includes("fetch")) {
        throw new Error(
          `Error de conexión con el servidor en ${API_BASE_URLS.user}`
        );
      }
...
export const catalogAPI = {
  ...
  async getYachtById(id: number): Promise<Yacht> {
    const response = await fetch(
      `${API_BASE_URLS.catalog}/api/catalog/yachts/${id}`,
      {
        signal: AbortSignal.timeout(8000), // 8 segundos de timeout
      }
    );
  ...
  async searchYachts(params: {
    ...
    const response = await fetch(
      ...
      }/api/catalog/yachts/search?${searchParams.toString()}`
    );
...
export const bookingAPI = {
  ...
  async confirmPayment(id: number, token: string): Promise<Booking> {
    const response = await fetch(
      `${API_BASE_URLS.booking}/api/bookings/${id}/pay`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
